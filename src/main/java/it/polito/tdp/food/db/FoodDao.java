...
public class FoodDao {
	...
	public List<Adiacenza> getAdiacenze() {
		...
		String sql = "select p1.portion_display_name as porz1, p2.portion_display_name as porz2, count(*) as peso "
				+ "from portion p1, portion p2 " + "where p1.food_code = p2.food_code "
				+ "AND p1.portion_display_name <> p2.portion_display_name " + "group by porz1,porz2";
		...
		try {
			...
			while (res.next()) {
				...
				String p1 = res.getString("porz1");
				String p2 = res.getString("porz2");
				Integer peso = res.getInt("peso");

				adiacenze.add(new Adiacenza(p1, p2, peso));
	...
	public List<String> getPorzioni(Integer calorie) {
		...
		String sql = "select distinct portion_display_name as porzione " + "from portion " + "where calories < ? order by portion_display_name ASC";
		...
		try {
			...
			while (res.next()) {
				porzioni.add(res.getString("porzione"));
			}
